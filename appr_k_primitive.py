...
def appr_k_primitive(matrix_name: str, main_matrix: Matrix) -> Matrix:
    """
       Given a 4x4 matrix M, this function modify the diagonal of the matrix M so that the new matrix M(prim)
       is K_primitive Mueller matrix.
       Using the fact that if a matrix M is Mueller, then 1/n*E_{11}+M is a K-primitive matrix.
       case 1: The matrix is a K-primitive Mueller matrix. Then M(prim)=M
       Case 2: The matrix M is a Mueller matrix but is not K-primitive, then M(prim)=1/n*E_11+M.
       Case 3: The matrix M is not a Mueller matrix, then M(prim)=1/n*E_11+M(mu).
       Parameters:
           -main_matrix (sympy 4x4 matrix): The matrix we want to approximate to a K-primitive Mueller matrix.

       Returns:
           Matrix: A sympy 4x4 matrix that is a K-primitive Mueller matrix.
       """
